...
This example provides a comprehensive demonstration of Port Peer's functionality.
...
3. Use the `Peer` to call the `hello_world` function and await resolution.
...
5. Instantiate a `Peer` in the worker thread.
6. Use the `Peer` to register the `hello_world` function in the worker.
7. Use the `Peer` to register the `a_reasonable_assertion` function in the worker.
8. Use the `Peer` to call a `very_late_binding` function in the main thread _that is not yet registered_.
9. Use the `Peer` to call the function registered as `hello_world` and await resolution.
...
17. Use the `Peer` to register a `very_late_binding` function in the main thread and log the long disposed thread's Id.
...
```bash
cd port-peer/examples/comprehensive
```
